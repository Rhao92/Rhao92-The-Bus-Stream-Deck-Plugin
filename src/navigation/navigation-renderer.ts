...
function distance(value: number | undefined, approximate = false): string {
  ...
  const formatted = clamped < 1_000
    ...
    : `${(clamped / 1_000).toFixed(1).replace(".", ",")} km`;
...
function valuePanel(
  ...
  const size = value.length >= 9 ? 28 : value.length >= 7 ? 33 : 41;
  const content = `<g filter="url(#valueGlow)">${valueIcon(kind, color)}</g>
  <text x="72" y="112" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${size}" font-weight="900" fill="#fff" filter="url(#valueGlow)">${escapeXml(value)}</text>`;
...
function maneuverLabel(kind: ManeuverKind): string {
  switch (kind) {
    ...
    case "recalculating": return "BERECHNUNG";
...
function statusLabel(status: GuidanceStatus): string {
  switch (status) {
    ...
    case "loading-map": return "KARTE LÄDT";
    case "no-route": return "KEINE ROUTE";
    case "stale-route": return "ROUTE ALT";
    case "off-route": return "ABSEITS ROUTE";
    case "reversing": return "RÜCKWÄRTS";
...
export function renderNavigationKey(
  ...
  if (model.status === "offline") return renderOfflineKey();
  ...
  if (kind === "maneuver") {
    ...
    const title = maneuver
      ? `NAVIGATION: ${maneuverLabel(maneuver.kind)} IN ${value}; ${maneuver.id}`
      : `NAVIGATION: ${statusLabel(model.status)}`;
  ...
  if (kind === "next-stop") {
    ...
    const content = `<circle cx="72" cy="25" r="14" fill="#061018" stroke="${color}" stroke-width="4" filter="url(#valueGlow)"/><text x="72" y="34" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="24" font-weight="900" fill="${color}">H</text>${body}<text x="72" y="121" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="13" font-weight="900" fill="${color}">${escapeXml(stopDistance)}</text>`;
    return dataUri(frame(
      content,
      color,
      color,
      `NÄCHSTER HALT: ${available ? model.nextRelevantStop : "--"}; ${stopDistance}`
    ));
  }
  if (kind === "total-distance") {
  ...
  if (kind === "route-progress") {
    ...
    return dataUri(frame(content, color, color, `LINIENFORTSCHRITT: ${value}`));
  }
  if (kind === "eta") {
    ...
    return dataUri(frame(content, color, color, `GESCHÄTZTE ANKUNFT ETA: ${eta}`));
  }
  if (kind === "predicted-delta") {
    ...
    return dataUri(frame(content, deltaColor, deltaColor, `PROGNOSE-DELTA: ${value}`));
  }

  const confidence = available ? model.predictionConfidence : "none";
  const confidenceLabel = confidence === "high"
    ? "HOCH"
    : confidence === "medium" ? "MITTEL" : confidence === "low" ? "NIEDRIG" : "--";
  ...
  return dataUri(frame(
    ...
    color,
    color,
    `PROGNOSESICHERHEIT: ${confidenceLabel}; ${status}`
  ));
}
