...
export const signedValue = (value: number): string =>
  value > 0 ? `+${value}` : `${value}`;
...
function stopRequest(bright: boolean): string {
  ...
  <text x="706" y="62" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="7" font-weight="800" letter-spacing=".5" fill="#ff7782">HALTEWUNSCH</text>
...
function timetable(
  ...
  const status = stop
    ? stopRequest(bright)
    : `<text x="700" y="61" class="fpLabel" fill="${accent}">STATUS</text><text x="700" y="84" class="fpStatus" fill="${accent}">${escapeXml(view.connectionLabel === "LIVE" ? view.status : view.connectionLabel)}</text>`;
  ...
  <text x="400" y="15" class="fpTop" fill="${routeStatus}">NÄCHSTE HALTESTELLE</text>
  ...
  ${metric(90, "ANK", view.arrival, accent)}
  ${metric(250, "ABF", view.departure, accent)}
  ${metric(410, "ABWEICHUNG", view.deltaText, delta, true)}
  ${metric(558, "INGAME ZEIT", view.ingameTime, accent)}
  ${status}
  <text x="789" y="12" text-anchor="end" font-family="Arial,Helvetica,sans-serif" font-size="5" font-weight="700" fill="#fff" fill-opacity=".28">2.15 BETA</text>`;
}

function vehicle(view: FullpanelViewModel): string {
  const battery = view.batteryPercent === undefined ? "–" : `${view.batteryPercent}%`;
  const batteryWidth = view.batteryPercent === undefined
    ? 0
    : Math.round(80 * view.batteryPercent / 100);
  const online = view.connectionLabel === "LIVE";
  const accent = online ? "#38c9ff" : "#8d96a3";
  const layerLabel = online ? "ECITARO · LIVE-TELEMETRIE" : view.connectionLabel;
  const speed = online ? view.speed : "--";
  const allowedSpeed = online && view.allowedSpeed > 0 ? view.allowedSpeed : "--";
  const neutralValue = "#d2d6dc";
  const doorColor = !online ? neutralValue : view.doors === "OFFEN" ? "#ff4050" : "#8fd7ee";
  const brakeColor = !online ? neutralValue : view.parkingBrake === "AN" ? "#ff4050" : "#8a949c";
  const kneelingColor = !online
    ? neutralValue
    : view.mechanicalKneeling === "AKTIV"
    ? "#ff4050"
    : view.mechanicalKneeling === "AUS" || view.mechanicalKneeling === "–"
      ? "#8a949c"
      : "#f4c842";
  ...
  <text x="20" y="16" font-family="Arial,Helvetica,sans-serif" font-size="8" font-weight="800" letter-spacing=".6" fill="${accent}">FAHRZEUG</text>
  ...
  <text x="224" y="91" text-anchor="middle" class="fpSmall" fill="${online ? "#9fdfff" : neutralValue}">STRECKENLIMIT</text>
  ...
  <text x="340" y="31" text-anchor="middle" class="fpLabel" fill="${accent}">GANG</text>
  ...
  <text x="480" y="31" text-anchor="middle" class="fpLabel" fill="${accent}">AKKU</text>
  ...
  <text x="632" y="28" class="fpVehicleLabel">TÜREN</text><text x="770" y="28" text-anchor="end" class="fpVehicleValue" fill="${doorColor}">${view.doors}</text>
  <text x="632" y="48" class="fpVehicleLabel">FESTSTELLBREMSE</text><text x="770" y="48" text-anchor="end" class="fpVehicleValue" fill="${brakeColor}">${view.parkingBrake}</text>
  <text x="632" y="68" class="fpVehicleLabel">KNEELING</text><text x="770" y="68" text-anchor="end" class="fpVehicleValue" fill="${kneelingColor}">${view.mechanicalKneeling}</text>
  <text x="632" y="88" class="fpVehicleLabel">LEISTUNG</text><text x="770" y="88" text-anchor="end" class="fpVehicleValue" fill="${online ? "#7ee65b" : neutralValue}">${view.power}</text>`;
}

function navigationDistance(value: number | undefined): string {
  if (value === undefined || !Number.isFinite(value)) return "--";
  const safe = Math.max(0, value);
  return safe < 1_000
    ? `${Math.round(safe)} m`
    : `${(safe / 1_000).toFixed(1).replace(".", ",")} km`;
}

function navigationManeuverLabel(kind: ManeuverKind): string {
  switch (kind) {
    case "straight": return "GERADEAUS";
    case "slight-left": return "LEICHT LINKS";
    case "left": return "LINKS";
    case "sharp-left": return "SCHARF LINKS";
    case "slight-right": return "LEICHT RECHTS";
    case "right": return "RECHTS";
    case "sharp-right": return "SCHARF RECHTS";
    case "lane-left": return "SPUR LINKS";
    case "lane-right": return "SPUR RECHTS";
    case "uturn": return "WENDEN";
    case "stop": return "HALTESTELLE";
    case "destination": return "ENDHALT";
    default: return "--";
  }
}
...
function navigationStatus(
  ...
  switch (model.status) {
    case "live": return hasManeuver ? "AKTIV" : "UNSICHER";
    case "loading-map":
    case "stale-route":
    case "reversing": return "UNSICHER";
    case "no-route": return "KEINE ROUTE";
    case "off-route": return "ROUTE VERLOREN";
    case "no-vehicle": return "NICHT IM BUS";
    case "bus-not-ready": return "BUS WIRD GELADEN";
...
function navigation(model: RouteGuidanceModel | undefined): string {
  if (!model || model.status === "offline") return offline();
  ...
  const tint = model.status === "off-route"
    ...
    : uncertain
      ...
      : model.status === "live" && maneuver
        ? "#061018"
        : "#0b0f14";
  const status = navigationStatus(model, Boolean(maneuver));
  const maneuverKind = maneuver?.kind ?? "unavailable";
  const maneuverLabel = navigationManeuverLabel(maneuverKind);
  const maneuverDistance = navigationDistance(maneuver?.distance);
  const stopDistance = model.status === "live"
    ? navigationDistance(model.nextRelevantStopDistance)
    : "--";
  ...
  const statusSize = status.length >= 15 ? 12 : status.length >= 11 ? 14 : 17;
  ...
  <rect x="6" y="6" width="788" height="88" rx="8" fill="#02070b" fill-opacity=".76" stroke="#fff" stroke-opacity=".05"/>
  <text x="20" y="16" font-family="Arial,Helvetica,sans-serif" font-size="8" font-weight="800" letter-spacing=".6" fill="${color}">NAVIGATION</text>
  <line x1="180" y1="20" x2="180" y2="88" class="fpDivider"/>
  <line x1="430" y1="20" x2="430" y2="88" class="fpDivider"/>
  <line x1="640" y1="20" x2="640" y2="88" class="fpDivider"/>
  <g data-active-maneuver-id="${escapeXml(maneuver?.id ?? "")}" transform="translate(58 7) scale(.7)" filter="${filter}">${navigationManeuverIcon(maneuverKind, color)}</g>
  <g data-active-maneuver-distance="${escapeXml(maneuver?.distance ?? "")}">
    <text x="305" y="32" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${maneuverSize}" font-weight="850" fill="${color}">${escapeXml(maneuverLabel)}</text>
    <text x="305" y="73" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="34" font-weight="900" fill="#fff" filter="${filter}">${escapeXml(maneuverDistance)}</text>
  </g>
  <text x="535" y="29" text-anchor="middle" class="fpLabel" fill="${color}">NÄCHSTER HALT</text>
  <text x="535" y="52" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${stopSize.toFixed(1)}" font-weight="800" fill="#fff">${escapeXml(stopName)}</text>
  <text x="535" y="79" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="21" font-weight="900" fill="${color}">${escapeXml(stopDistance)}</text>
  <text x="715" y="34" text-anchor="middle" class="fpLabel" fill="${color}">NAV-STATUS</text>
  <text x="715" y="66" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${statusSize}" font-weight="900" fill="${color}" filter="${filter}">${escapeXml(status)}</text>
  <text x="789" y="12" text-anchor="end" font-family="Arial,Helvetica,sans-serif" font-size="5" font-weight="700" fill="#fff" fill-opacity=".28">2.15 BETA</text>`;
}
...
function offline(): string {
  ...
  <text x="400" y="58" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="34" font-weight="900" letter-spacing="4" fill="#d2d6dc">OFFLINE</text>
  <text x="400" y="78" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="9" font-weight="700" letter-spacing="1" fill="#8d96a3">SPIEL / TELEMETRIE NICHT VERBUNDEN</text>`;
}
...
export function renderFullpanel(
  ...
  const content = layer === "timetable"
    ? timetable(view, manualStop, bright)
    : vehicle(view);
...
export function renderFullpanelSetup(column: number, count: number): string {
  ...
  <text x="100" y="25" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="10" font-weight="800" fill="#fff">FULLPANEL</text>
  <text x="100" y="48" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="9" font-weight="700" fill="#f4b942">SEGMENT ${segment}</text>
  <text x="100" y="69" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="8" fill="#fff0c4">ALLE 4 ENCODER BELEGEN</text>
  <text x="100" y="85" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="8" fill="#c9b892">ERKANNT: ${safeCount} / 4</text>
</svg>`);
}
